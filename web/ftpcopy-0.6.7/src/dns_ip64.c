...
static int dns_ip64_packet_add(stralloc *out,char *buf,unsigned int len)
{
  unsigned int pos;
  char header[16];
  uint16 numanswers;
...
int dns_ip64_packet(stralloc *out,char *buf,unsigned int len) {
  if (!stralloc_copys(out,"")) return -1;
  return dns_ip64_packet_add(out,buf,len);
}

static char *q = 0;

int dns_ip64(stralloc *out,stralloc *fqdn)
{
  unsigned int i;
  char code;
  char ch;
  ...
  for (i = 0;i <= fqdn->len;++i) {
    ...
    if (dns_resolve(q,DNS_T_AAAA) != -1)
      if (dns_ip64_packet_add(out,dns_resolve_tx.packet,dns_resolve_tx.packetlen) != -1) {
