...
static int init(char ip[256])
{
  ...
  if (!iplen) {
    byte_copy(ip,sizeof V6loopback, V6loopback);
    byte_copy(ip+16,sizeof V4mappedprefix, V4mappedprefix);
    byte_copy(ip+16+sizeof V4mappedprefix,sizeof V4loopback,V4loopback);
    iplen = 32;
  }
